import svgPaths from "./svg-gkcc8oftuu";
  ...
  return (
    ...
    >
      <div className="font-['PT Sans',_'Helvetica_Neue',_'Arial',_sans-serif] font-bold leading-[0] not-italic relative shrink-0 text-[#2d2b2b] text-[15px] text-center text-nowrap tracking-[-0.43px]">
        <p className="adjustLetterSpacing block leading-[16px] whitespace-pre">Start</p>
      </div>
    </div>
  );
}
...
function Container() {
  return (
    <div
      className="absolute box-border content-stretch flex flex-col gap-5 items-start justify-start leading-[0] left-[22px] p-0 text-center top-[254px] w-[351px]"
      data-name="Container"
    >
      <div className="font-['Kreon:Regular',_sans-serif] font-normal relative shrink-0 text-[#cfcfcf] text-[36px] w-full">
        <p className="block leading-[0.8] mb-0">{`You don't have `}</p>
        <p className="leading-[0.8]">
          <span className="text-[#e1ff00]">to cope</span>
          <span>{` alone`}</span>
        </p>
      </div>
      <div className="font-['PT Sans',_'Helvetica_Neue',_'Arial',_sans-serif] h-[89px] not-italic relative shrink-0 text-[#ffffff] text-[20px] w-full">
...
export default function WellcomeThemeOpenedPage() {
  return (
    <div className="bg-[#111111] relative size-full" data-name="Wellcome Theme opened  page">
      <Light />
      <Button />
      <Container />
      <MiniStripeLogo />
