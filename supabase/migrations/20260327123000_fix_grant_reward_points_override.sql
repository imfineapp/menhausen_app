-- Security hardening: do not trust client-asserted points for fixed-point rules.
-- For payload-driven rules (points = 0), allow only bounded positive payload points.
...
BEGIN
  ...
  IF v_rule.points > 0 THEN
    -- Fixed-point rules are fully server-authoritative.
    v_points := v_rule.points;
  ELSE
    -- Payload-driven rules are allowed only with strict positive cap.
    v_points := LEAST(GREATEST(COALESCE(v_payload_points, 0), 0), 500);
  END IF;
