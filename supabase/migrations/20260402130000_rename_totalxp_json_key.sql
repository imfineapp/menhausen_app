-- Rename achievements JSON key from totalXP to totalPointsFromAchievements
  ...
  SELECT json_build_object(
    ...
    'achievements', (
      SELECT json_build_object(
        ...
        'totalPointsFromAchievements', COALESCE(ua.total_xp, 0),
    ...
    'experimentAssignment', (
      SELECT json_build_object(
        'experimentKey', ea.experiment_key,
        'variant', ea.variant,
        'assignedAt', ea.assigned_at
      )
      FROM experiment_assignments ea
      WHERE ea.telegram_user_id = p_telegram_user_id
        AND ea.experiment_key = 'onboarding_flow_v1'
    ),

    'topicTestResults', COALESCE(
      (SELECT json_object_agg(
        ttr.topic_id,
        json_build_object(
          'topicId', ttr.topic_id,
          'answers', COALESCE(ttr.answers, '[]'::jsonb),
          'score', COALESCE(ttr.score, 0),
          'percentage', COALESCE(ttr.percentage, 0),
          'completedAt', ttr.completed_at
        )
      )
      FROM topic_test_results ttr
      WHERE ttr.telegram_user_id = p_telegram_user_id),
      '{}'::json
    ),
