...
function formatArticleContent(content: string): string {
  // Parse into blocks (headings / lists / paragraphs) to avoid invalid <p> nesting
  // and inconsistent spacing when content starts with headings.
  ...
  const flushList = () => {
    ...
    out.push(
      `<ul class="list-disc list-inside mb-4 space-y-2">${listItems.join('')}</ul>`
    );
    listItems = [];
  };
  ...
  const emitHeading = (level: 1 | 2 | 3, text: string) => {
    ...
    if (level === 1) {
      out.push(`<h1 class="typography-h1 text-[#e1ff00] mt-8 mb-4">${t}</h1>`);
    } else if (level === 2) {
      out.push(`<h2 class="typography-h2 text-[#e1ff00] mt-8 mb-4">${t}</h2>`);
    } else {
      out.push(`<h3 class="typography-h3 text-[#e1ff00] mt-6 mb-3">${t}</h3>`);
    }
  };
